...
function renderSanskritHeader(isRu, query, isFallback) {
    let title = getUiText('sanskrit');
    if (isFallback) {
        const fallbackNotice = `слово: <b>${query}</b>`;
        title += ` <span style="font-size: 0.8em; font-weight: normal; color: #666;">(${fallbackNotice})</span>`;
    }
    
    const extraHtml = `<button id="sanskrit-toggle-all" style="background:none; border:none; color:#999; font-size: 1.1em; font-weight:bold; cursor:pointer;" title="Toggle All" onclick="event.stopPropagation();">+/-</button>`;
    
    return renderExtDictHeader(title, null, extraHtml);
}
